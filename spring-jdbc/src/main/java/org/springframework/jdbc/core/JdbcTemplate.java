...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		//fetchSize 主要是减少网络交互次数设计的.访问ResultSet时,如果每次只从服务器读取一行,会有大量的开销
		//setFetchSize的意思是,当调用rs.next时,ResultSet会一次性从服务器上取得多少行数据回来,这样下次rs.next时,直接从内存中读取
		//这个设置可能会被有些jdbc驱动忽略,并且设置过大会造成内存的上升
		int fetchSize = getFetchSize();
		if (fetchSize != -1) {
			stmt.setFetchSize(fetchSize);
		}
		//maxRows设置限定此Statement对象生成的ResultSet对象包含的最大行数
	...
	protected void handleWarnings(Statement stmt) throws SQLException {
		if (isIgnoreWarnings()) {//忽略警告就只是打印警告
			if (logger.isDebugEnabled()) {
