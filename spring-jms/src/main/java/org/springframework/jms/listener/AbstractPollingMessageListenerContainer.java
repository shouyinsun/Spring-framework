...
public abstract class AbstractPollingMessageListenerContainer extends AbstractMessageListenerContainer {
	...
	protected boolean receiveAndExecute(
		...
		if (this.transactionManager != null) {//配置了transactionManager 事务管理器 具有事务
			...
			catch (JMSException | RuntimeException | Error ex) {//异常回滚由事务管理器的rollback方法处理
				//异常执行transactionManager的rollback方法
