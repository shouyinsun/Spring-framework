...
public class DefaultMessageListenerContainer extends AbstractPollingMessageListenerContainer {
	...
	protected void doInitialize() throws JMSException {
		synchronized (this.lifecycleMonitor) {
			/**
			 * 并发consumer数量
			 * concurrentConsumers 属性指定允许创建多少个session和MessageConsumer来接收消息
			 * 只有在 Destination 为queue的时候才能使用多个messageConsumer
			 * 因为queue中的消息只能被一个consumer接收
			 * 可以提高消息处理性能但消息顺序得不到保证
			 *
			 * 不应该在Destination 为topic时 使用多个MessageConsumer,因为每个MessageConsumer会接收到相同的消息
			 */

			//根据concurrentConsumers 数量开启对应数量的线程 放入线程池执行
	...
	private class AsyncMessageListenerInvoker implements SchedulingAwareRunnable {
		...
		public void run() {
			...
			try {
				//每个任务设置的最大处理消息数量
				if (maxMessagesPerTask < 0) {//小于0 默认无限制,一直接收消息
			...
			while (active) {
				synchronized (lifecycleMonitor) {
					...
					while ((active = isActive()) && !isRunning()) {
						...
						if (!wasWaiting) {//如果并非等待状态说明while是第一次执行,将激活数量减1
							decreaseActiveInvokerCount();
						}
						//等待状态标志
						wasWaiting = true;
						/**
                         * 这里不能单纯的使用while循环,因为会造成资源的巨大浪费
						 * 全局变量 lifecycleMonitor wait()方法来暂停线程
						 *
						 * 所以如果线程需要再次恢复的话,不仅需要更改this.running
						 * 还需要 lifecycleMonitor.notify() 或者 notifyAll()
						 */
						try {//进入等待状态,等待恢复命令 等待notify 或者 notifyAll
		...
		private boolean invokeListener() throws JMSException {
			//初始化资源 包括首次创建session 和 consumer
