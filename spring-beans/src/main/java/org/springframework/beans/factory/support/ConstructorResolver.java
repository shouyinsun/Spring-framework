...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(final String beanName, final RootBeanDefinition mbd,
		...
		//explicitArgs 明确的参数,通过getBean方法传入
		...
		if (explicitArgs != null) {
			...
			//如果getBean方法中没有指定参数,则尝试从配置文件中解析
		...
		if (constructorToUse == null) {
			...
			//构造函数排序,public 构造函数优先,参数个数降序 non public 参数个数降序
			...
			for (Constructor<?> candidate : candidates) {
				...
				//已经找到选用的构造函数,或者需要的参数个数小于当前的构造函数参数个数,则终止
				// 因为已经按照参数个数降序排序
				...
				if (resolvedValues != null) {//有参数则根据值构造对应参数类型的参数
					try {
						//获取@ConstructorProperties注解的参数名
			...
			if (explicitArgs == null) {//解析的构造函数加入缓存
