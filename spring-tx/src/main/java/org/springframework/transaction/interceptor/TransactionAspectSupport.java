...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		//构造方法唯一标识(类.方法) (clazz != null ? clazz : method.getDeclaringClass()).getName() + '.' + method.getName();
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		//txAttr 中没有name则使用方法唯一标识,并使用DelegatingTransactionAttribute封装
		...
		if (txAttr != null) {
			if (tm != null) {
				//获取事务status
				//注入的DataSourceTransactionManager extends AbstractPlatformTransactionManager
		...
		//根据指定的属性与status构建 TransactionInfo
		//一旦事务执行失败,会根据TransactionInfo 信息进行回滚相关操作
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			//当前存在事务  并且是 RuntimeException 或者 ERROR
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
				try {
					//根据TransactionInfo信息进行回滚操作
