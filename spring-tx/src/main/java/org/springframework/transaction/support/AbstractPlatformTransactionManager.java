...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		//判断当前线程是否存在事务  DataSourceTransactionManager的判断依据是
		//当前线程记录的连接不为空,且连接中的transactionActive 属性为true
		...
		//如果当前线程不存在事务,但是 PropagationBehavior被申明为 PROPAGATION_MANDATORY 则抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		//PROPAGATION_REQUIRED PROPAGATION_REQUIRES_NEW PROPAGATION_NESTED 这些都需要新建事务
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				//可以使用保存点的方式控制事务回滚,那么在嵌套事务建立初始建立保存点
		...
		if (isValidateExistingTransaction()) {//可验证已存在的事务
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		//事务链中已经被标记回滚,那么就不会尝试提交,直接回滚
		//异常回滚操作的 processRollback 中进行设置
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		 ...
		 * 当前非新事务也不会进行提交操作
		 *
		 *  主要是考虑嵌套事务,嵌套事务开始之前设置保存点,一旦内嵌的事务发生异常,根据保存点信息回滚,
		 *  如果没有异常,内嵌事务并不会单独提交,而是根据事务流由最外层事务负责提交,
		 *  所以如果当前存在保存点信息便不是最外层事务,不做保存操作
		 *  非新事务同理
		 */
		try {
			...
			try {
				...
				if (status.hasSavepoint()) {//清除保存点信息
					if (status.isDebug()) {
				...
				else if (status.isNewTransaction()) {//新的事务直接提交
					if (status.isDebug()) {
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				//自定义触发器的调用,激活所有的TransactionSynchronization 的 beforeCompletion 方法
				triggerBeforeCompletion(status);
				...
				if (status.hasSavepoint()) {
					...
					//常用于嵌套事务,对于嵌套事务,内嵌事务异常并不会引起外部事务的回滚
					...
					if (status.hasTransaction()) {
						//存在事务 又不是上面两种情况,多数用于JTA
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							if (status.isDebug()) {
								logger.debug("Participating transaction failed - marking existing transaction as rollback-only");
							}
							//当前事务不是独立事务,只能标记状态,等到事务链执行完毕后统一回滚
			...
			//自定义触发器的调用,激活TransactionSynchronization 的 beforeCompletion 方法
			triggerAfterCompletion(status, TransactionSynchronization.STATUS_ROLLED_BACK);
	...
	private void cleanupAfterCompletion(DefaultTransactionStatus status) {
		//设置状态 完成
		status.setCompleted();
		if (status.isNewSynchronization()) {//当前事务是新的同步状态,需要将绑定到当前线程的事务信息清除
			TransactionSynchronizationManager.clear();
		}
		if (status.isNewTransaction()) {//新事务清理资源
			doCleanupAfterCompletion(status.getTransaction());
		}
		if (status.getSuspendedResources() != null) {//在事务执行之前有事务挂起,那么当前事务执行结束后需要将挂起事务恢复
			if (status.isDebug()) {
