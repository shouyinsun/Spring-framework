...
public class RmiClientInterceptor extends RemoteInvocationBasedAccessor
	...
	public void afterPropertiesSet() {
		super.afterPropertiesSet();//Property 'serviceUrl' is required"
		prepare();
	}
	...
	public void prepare() throws RemoteLookupFailureException {
		...
		// 如果配置了 lookupStubOnStartup 会在启动时寻找stub
		if (this.lookupStubOnStartup) {
			//获取stub
			Remote remoteObj = lookupStub();
			if (logger.isDebugEnabled()) {
				//RmiInvocationWrapper 实现 RmiInvocationHandler
				// 在 ServiceExport的getObjectToExport()方法中封装 RmiInvocationWrapper
	...
	protected Remote lookupStub() throws RemoteLookupFailureException {
		try {//两种方式获取stub
			...
			else {//2.直接使用RMI提供的标准方法 Naming.lookup(getServiceUrl())
	...
	protected Object doInvoke(MethodInvocation invocation, Remote stub) throws Throwable {
		if (stub instanceof RmiInvocationHandler) {//服务端使用 RmiInvocationWrapper 封装
			// RMI invoker
			try {
	...
	protected Object doInvoke(MethodInvocation methodInvocation, RmiInvocationHandler invocationHandler)
		...
		//将methodInvocation信息封装到 RemoteInvocation 并通过远程代理的方法直接调用
