...
public class RmiServiceExporter extends RmiBasedExporter implements InitializingBean, DisposableBean {
	...
	public void prepare() throws RemoteException {
		...
		/**
		 * 用户可以自定义4个套接字工厂配置
		 * clientSocketFactory  serverSocketFactory registryClientSocketFactory registryServerSocketFactory
		 *
		 * registryClientSocketFactory 跟  registryServerSocketFactory 用于主机跟RMI服务器之间建立连接
		 * getRegistry 获取Registry实例时, RMI主机使用registryServerSocketFactory 创建socket等待连接 服务端使用 registryClientSocketFactory 建立socket发起连接
		 *
		 * clientSocketFactory 跟 serverSocketFactory 用于导出远程对象
		 * serverSocketFactory 用于服务端建立socket等待连接客户端连接  clientSocketFactory 用于调用端建立socket发起连接
		 *
		 */

		// Check socket factories for exported object.
		//如果在配置文件中配置了clientSocketFactory 或者 serverSocketFactory 的处理
		/**
		 * 如果配置中的 clientSocketFactory 同时又实现了 RMIServerSocketFactory
		 * 会忽略 serverSocketFactory 的设置,直接使用 clientSocketFactory
		 */
		if (this.clientSocketFactory instanceof RMIServerSocketFactory) {
			this.serverSocketFactory = (RMIServerSocketFactory) this.clientSocketFactory;
		}
		//clientSocketFactory 与 serverSocketFactory 要么同时出现要么同时不出现
		...
		// registryClientSocketFactory 与  registryServerSocketFactory 配置
		...
		//初始化导出的object 通常是 RmiInvocationWrapper 封装的jdk代理类
		//当客户端调用serviceName的服务时,会转到 RmiInvocationWrapper 的invoke方法
		...
		try {
			if (this.replaceExistingBinding) {
				...
				//绑定服务名称到remote object,外界调用serviceName时就会被exportObject 接收
	...
	protected Registry getRegistry(String registryHost, int registryPort,
		...
		if (registryHost != null) {//RMI的注册主机与发布的服务不在同一台机器
			// Host explicitly specified: only lookup possible.
			if (logger.isInfoEnabled()) {
				logger.info("Looking for RMI registry at port '" + registryPort + "' of host [" + registryHost + "]");
			}
			//registryHost 不为空 则尝试获取对应主机的 Registry
	...
	protected Registry getRegistry(int registryPort,
		...
		if (clientSocketFactory != null) {//使用 clientSocketFactory 创建Registry
			...
			synchronized (LocateRegistry.class) {
				try {
					...
					// 复用测试,如果没有在 testRegistry 抛出异常 然后在 catch 里面创建
